...
ROOT=Path('/mnt/data/SSZ_P5_FULL_CLOSURE_CODEX_HANDOFF_2026-09-16')
PAPER=ROOT/'paper'
ledger=pd.read_csv(ROOT/'data/certificates/SSZ_P5_FULL_CLOSURE_GATE_LEDGER_2026-09-16.csv')
audit=json.loads((ROOT/'data/certificates/SSZ_P5_FULL_CLOSURE_AUDIT_2026-09-16.json').read_text())
...
slot_groups={
'a':('metric-scalar constraint family',9),
'b':('metric derivative/mixing family',5),
'c':('scalar radial/potential family',6),
'd':('scalar kinetic/gradient family',4),
'e':('scalar background-derivative family',4),
'v':('vector and scalar-vector family',13),
}
slot_notes={
'a1':'primary scalar-metric derivative mixing coefficient; enters the generalized psi definition and the H1/H2 constraint algebra.',
'a2':'radial derivative partner of a1; its holonomic relation to a1 is essential for the corrected a5 identity.',
'a3':'metric constraint coefficient appearing in the denominators of the generalized psi map; must stay nonzero on the reduced domain.',
'a4':'central geometric coefficient controlling P1, the H0 constraint structure, and several exact Appendix identities.',
'a5':'lower-order scalar slot. Production identity: a5=a2\' - a1\'\' -(A0\' v4/2)\' + A0\'v5/2; pure Horndeski reduces to a2\'-a1\'\'.',
'a6':'finite-l scalar coefficient entering e1 and the no-ghost combination after constraint reduction.',
'a7':'metric-scalar algebraic coefficient entering the H0 constraint and finite-l terms.',
'a8':'multipole-dependent metric-scalar coefficient appearing together with L in the algebraic constraints.',
'a9':'radial derivative completion of a4 and v6; useful as an Appendix-A regression identity.',
'b1':'coefficient fixed algebraically by a4/(2f) in the standard conventions.',
'b2':'mixing coefficient sensitive to the action representative and background derivatives.',
'b3':'finite-l derivative mixing; independently checked in the reconstructed Horndeski emitter.',
'b4':'exact geometric identity proportional to 2 r a4/f.',
'b5':'exact geometric identity -a4/f.',
'c1':'vanishes in the Zhang-Kase gauge-fixed coefficient dictionary.',
'c2':'principal scalar radial-control slot. It is a convenient target for background-null transverse action jets.',
'c3':'lower-order scalar potential slot; selected from the action-level representative rather than guessed pointwise.',
'c4':'Horndeski principal-control target and scalar-metric radial mixing coefficient.',
'c5':'finite-l scalar gradient/potential completion involving a4,d2,v6 and odd-sector data.',
'c6':'finite-l scalar completion; an earlier parser error in the rf\'/f factor was corrected on 2026-09-16.',
'd1':'exactly a4/(2f) in the standard Appendix dictionary.',
'd2':'scalar radial coefficient entering e2 and c5/c6.',
'd3':'contains holonomic partial_phi v6; production uses on-curve derivative (dv6/dr)/phi\'.',
'd4':'equals -2 alpha6 and links the even dictionary to the odd gravitational stability coefficient.',
'e1':'direct no-ghost identity carrier. In the ZK sector e1 sqrt(fh) phi\'^2 equals the exact even kinetic combination.',
'e2':'algebraic scalar constraint coefficient built from c2,d2 and v4.',
'e3':'one half of partial E_phi/partial phi; lower-order action slot selected from the on-shell representative.',
'e4':'profile-derivative coefficient involving a4,c4,v6,v9,v10,v13 and odd-sector data; derivative sensitive.',
'v1':'quadratic electric/vector kinetic slot. For the final epsilon-Y deformation it receives the direct genuine-SVT correction.',
'v2':'H0-vector mixing. On the final zero-vector background deformation it remains unchanged.',
'v3':'H2-vector mixing; related to v2,v4,v6 by Appendix identities.',
'v4':'scalar-gradient/vector mixing; a background-null transverse control direction in general SVT.',
'v5':'lower-order scalar/vector mixing. It vanishes for constant epsilon Y on A0\'=0.',
'v6':'odd/even bridge coefficient v6=4 alpha2. Production branch fixes v12=-v6/(2h).',
'v7':'auxiliary-vector quadratic completion. After action-level sector addition recanonicalize v7=v2^2/(4v1).',
'v8':'finite-l vector algebraic coefficient determined by v6,v10 and geometry.',
'v9':'equals alpha4, the odd vector no-ghost coefficient.',
'v10':'equals alpha5, the odd vector radial-gradient coefficient. It is changed by epsilon Y.',
'v11':'equals -v6/2.',
'v12':'accepted Appendix branch v12=-v6/(2h); the historical plus branch is superseded.',
'v13':'derivative-sensitive vector completion; JET9D8 regression is interior-accurate and edge-sensitive.',
}

preamble=r'''\documentclass[11pt,a4paper,oneside]{book}
...
ch('Executive closure statement',r'''
...
ch('What is meant by full constructive closure',r'''
...
ch('Frozen P5 geometry and scalar variable',r'''
...
ch('The regime variable kappa and the natural background basis',r'''
...
ch('Global Horndeski P5 reconstruction',r'''
...
ch('Regular quartic-Horndeski core and analytic center',r'''
...
ch('Finite-ell Maxwell-Horndeski no-ghost theorem as a regression oracle',r'''
...
ch('Historical genuine-SVT local witness',r'''
...
ch('The final genuine-SVT deformation: exact background-null proof',r'''
Consider
\begin{equation}
...
ch('Exact vector stability of the epsilon-Y member',r'''
...
\begin{figure}[H]\centering\includegraphics[width=.86\textwidth]{figures/epsY_ZA_strongH.png}\caption{$Z_A$ on the strong-field carrier.}\label{fig:ZAstrong}\end{figure}
\begin{figure}[H]\centering\includegraphics[width=.86\textwidth]{figures/epsY_ZA_core.png}\caption{$Z_A$ on the punctured core.}\label{fig:ZAcore}\end{figure}
...
ch('Unreduced 41-slot language',r'''
...
ch('JET9D8 derivative service and holonomicity',r'''
...
ch('Common constraint elimination',r'''
...
ch('Profile-aware operator and canonical K,G,S,M form',r'''
...
ch('Why the direct global KRGM export is a separate gate',r'''
...
ch('QNM threshold and recommended spectral formulation',r'''
...
ch('Error taxonomy and superseded routes',r'''
...
ch('Reproducibility policy and immutable release rules',r'''
...
ch('Codex repository blueprint',r'''
...
# Appendices: 41-slot dictionary
appendix='''\\appendix\n\\chapter{Complete 41-slot semantic dictionary}\\label{app:slots}\n'''
for prefix,(family,n) in slot_groups.items():
    appendix += f"\\section{{Family {prefix}: {family}}}\n"
    for i in range(1,n+1):
        key=f'{prefix}{i}'
        note=slot_notes.get(key,'Coefficient in the common unreduced even-parity action; regenerate from the accepted action emitter and preserve its exact Appendix identities.')
        note_tex=note.replace('_','\\_').replace('%','\\%').replace('&','\\&').replace('^','\\textasciicircum{}')
        appendix += f"\\subsection{{\\texttt{{{key}}}}}\n{note_tex}\n\n"
        appendix += "The release policy for this slot is to treat the covariant action and the accepted emitter as primary. A stored CSV value is a regression target only. During a handover the slot is obtained from the glued action jets, not from interpolation of already reduced matrices.\\par\n\n"

# Gate ledger longtable + one detailed paragraph per gate
appendix += r'''\chapter{Machine-readable closure gate ledger}\label{app:gates}
\small
\begin{longtable}{p{.28\textwidth}p{.12\textwidth}p{.14\textwidth}p{.36\textwidth}}
...
for _,r in ledger.iterrows():
    appendix += f"{esc(r['name'])} & {esc(r['status'])} & {esc(r['level'])} & {esc(r['criterion'])}; {esc(r['evidence'])}\\\\\n"
appendix += r'''\bottomrule\end{longtable}\normalsize
\section{Interpretation record}
'''
for _,r in ledger.iterrows():
    appendix += f"\\subsection{{{esc(r['name'])}}}\n"
    appendix += f"Status: \\textbf{{{esc(r['status'])}}}. Level: {esc(r['level'])}. Criterion: {esc(r['criterion'])}. Evidence source: \\texttt{{{esc(r['evidence'])}}}. "
    appendix += "This gate is evaluated independently in the Python auditor so that a future repository cannot inherit a PASS solely from prose. Where the gate is exact, the numerical files are supporting regressions; where it is numerical, the tolerance and source are part of the release contract.\\par\n\n"

# Code appendices
codefiles=[
('Full closure auditor','appendices/code_ascii/ssz_p5_full_closure_auditor.py'),
('Zhang--Kase 41-slot emitter','appendices/code_ascii/ssz_p5_zk_appendixA_emitter_JET9D8_2026-09-16.py'),
('Variable-$G_4$ Maxwell--Horndeski emitter','appendices/code_ascii/ssz_p5_mh_luminal_g4phi_emitter_JET9D8_2026-09-16.py'),
('JET9D8 higher-jet service','appendices/code_ascii/ssz_p5_higher_jet_closure_2026-09-16.py'),
('Holonomic $a_5$ closure','appendices/code_ascii/ssz_p5_holonomic_a5_closure_2026-09-16.py'),
('Full algebraic constraint maps','appendices/code_ascii/ssz_hybrid_full_constraint_maps_JET9D8(1).py'),
('Profile-aware operator reducer','appendices/code_ascii/ssz_p5_profile_operator_reducer_JET9D8_2026-09-16.py'),
('High-$L$ mass closure','appendices/code_ascii/ssz_p5_F2b_action_derived_highL_mass_closure_2026-09-15.py'),
('High-$L$ KGSM cross-check','appendices/code_ascii/ssz_p5_KGSM_highL_crosscheck_2026-09-16.py'),
('Common unreduced kernel','appendices/code_ascii/ssz_hybrid_unreduced_even_kernel.py'),
('First-stage jet-aware constraint reducer','appendices/code_ascii/ssz_hybrid_jet_aware_constraint_reducer.py'),
]
appendix += '\\chapter{Authoritative source listings}\nThe following listings are included verbatim from the release package. They are part of the reproducibility record; the prose derivation remains the conceptual specification.\n'
for title,path in codefiles:
    appendix += f"\\section{{{title}}}\n\\lstinputlisting[language=Python]{{{path}}}\n"

# Provenance PDFs
appendix += r'''\chapter{Historical provenance documents}
...
tex=preamble+'\n'.join(chapters)+appendix+bib
(PAPER/'SSZ_P5_FULL_CLOSURE_MONOGRAPH_2026-09-16.tex').write_text(tex,encoding='utf-8')
print('Wrote',PAPER/'SSZ_P5_FULL_CLOSURE_MONOGRAPH_2026-09-16.tex', 'chars',len(tex))
