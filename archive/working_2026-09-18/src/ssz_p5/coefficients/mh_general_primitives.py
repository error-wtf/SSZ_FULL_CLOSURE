...
def jet9d8(x: Iterable[float], y: Iterable[float], order: int = 1,
            window: int = 9, degree: int = 8) -> np.ndarray:
    """Local polynomial JET9D8 derivative on a monotone non-uniform grid."""
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    if x.ndim != 1 or y.ndim != 1 or len(x) != len(y):
        raise ValueError("x,y must be equal-length one-dimensional arrays")
    if window < degree + 1:
    ...
    for i in range(n):
        ...
        out[i] = (math.factorial(order) * co[order] / scale**order
                  if order <= deg else np.nan)
    return out
...
def emit_from_primitives(df: pd.DataFrame, *, window: int = 9, degree: int = 8,
                         regularize_photon_root: bool = False) -> pd.DataFrame:
    ...
    Q = (2.0 * phipp / (h * ph * r)
         + fp**2 / f**2 - fp * hp / (f * h)
         - 2.0 * fp / (f * r) + 2.0 * hp / (h * r)
         + hp / (h**2 * r))
    with np.errstate(divide="ignore", invalid="ignore"):
        d3 = (-((2.0 * phipp / ph + hp / h) * a1) / r**2
              + 2.0 * f / (photon * ph) * Q * a4
              + photon / (f * r) * da4_dphi
              + np.sqrt(f) / (ph * np.sqrt(h) * r**2) * F
              - f**1.5 / (np.sqrt(h) * photon * ph)
              * (fp / (f * r) + 2.0 * phipp / (ph * r)
                 + hp / (h * r) - 2.0 / r**2) * G)
    d4 = 0.5 * sqfh * G / r**2

    e1 = ((fp / f + 0.5 * hp / h) * a1 - 2.0 * a1p + a2
          - 2.0 * r * h * a6 - 0.5 * Ap * ov4) / (ph * f * h)
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        e4 = (c4p / ph
              ...
              * ((photon - 4.0 * f) * fp / (f**2 * r)
                 ...
                 - 4.0 * f * (2.0 * phipp * h + hp * ph)
                   / (ph * h**2 * r * photon))
              + 0.5 * hp * c4 / (h * ph)
              - 0.5 * photon * da4_dphi / (f * h * r * ph)
              + 0.5 * (fp * h * r - f) * F
                / (r**2 * np.sqrt(f) * ph**2 * h**1.5)
              + 0.5 * np.sqrt(f) * G / (r * ph**2 * h**1.5)
    ...
    out = pd.DataFrame({
        "u": _arr(d, "u", 1.0 / r), "x": r,
        "phi": _arr(d, "phi", np.nan), "f": f, "h": h,
        "phiprime": ph, "A0prime": Ap,
        ...
        "v6": np.zeros_like(r), "v7": ov6, "v8": ov7,
        "v9": ov8, "v10": ov9,
        "v11": np.zeros_like(r), "v12": np.zeros_like(r), "v13": np.zeros_like(r),
        "F_tensor": F, "G_tensor": G, "H_tensor": H,
