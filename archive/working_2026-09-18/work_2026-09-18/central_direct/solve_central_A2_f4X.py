...
def vals(i,F4X):
 rr=r[i];ff=f[i];hh=h[i];pp=ph[i];aa=Ap[i];FP=fp[i];FPP=fpp[i];HP=hp[i];APP=App[i];A4=a4[i];V6=v6[i];F4=prof.f4.to_numpy(float)[i]
...
f4new=prof.f4.to_numpy(float).copy(); f4Xnew=np.full(len(raw),np.nan); f3new=np.full(len(raw),np.nan); f2Fnew=np.full(len(raw),np.nan); v10new=np.full(len(raw),np.nan)
for i in ids:
 q0=vals(i,0.0); q1=vals(i,1.0); slope=q1[-1]-q0[-1]
 if abs(slope)<1e-12: raise RuntimeError(('tiny slope',i,slope))
 F4X=-q0[-1]/slope
 q=vals(i,F4X); f4Xnew[i]=F4X;f3new[i]=q[0];f2Fnew[i]=q[1];v10new[i]=q[2]
# metric solve f2,f2X
f2old=prof.f2.to_numpy(float);f2Xold=prof.f2X.to_numpy(float);A=r*r*f;kap=h*ph*ph
E00=r*f*hp-(f*(1-h)+r*r*(f*f2old-h*Ap*Ap*f2Fnew)-2*r*h*h*ph*Ap*Ap*f3new+h*Ap*Ap*(4*(h-1)*f4new-h*h*ph*ph*f4Xnew))
E11=r*h*fp-(f*(1-h)+r*r*(f*f2old+f*h*ph*ph*f2Xold-h*Ap*Ap*f2Fnew)-2*r*h*h*ph*Ap*Ap*(3*f3new-h*ph*ph*f3X)+h*Ap*Ap*(4*(3*h-1)*f4new-h*(9*h-4)*ph*ph*f4Xnew+h**3*ph**4*f4XX))
...
'f3':f3new[ids],'f3X':f3X[ids],'f3XX':hj.f3XX_selected.iloc[ids].to_numpy(),'tf3':prof.tilde_f3.iloc[ids].to_numpy(),'f4':f4new[ids],'f4X':f4Xnew[ids],'f4XX':hj.f4XX_recovered.iloc[ids].to_numpy(),'f4XXX':hj.f4XXX_selected.iloc[ids].to_numpy(),'tf4':0.0,'f3phi':f3phi,'f3phiX':hj.f3phiX.iloc[ids].to_numpy(),'f4phi':f4phi,'f4phiX':hj.f4phiX.iloc[ids].to_numpy()})
out0=zk.emit(base)
...
# background checks
F2=f2new[ids];F2X=f2Xnew[ids];F2F=f2Fnew[ids];F3=f3new[ids];F4=f4new[ids];F4X=f4Xnew[ids];F3X=f3X[ids];F4XX=f4XX[ids];fc=f[ids];hc=h[ids];pc=ph[ids];Ac=Ap[ids];fpc=fp[ids];hpc=hp[ids]
...
res=[]
for i in ids: res.append(vals(i,f4Xnew[i])[-1])
print('f4 fixed range',np.nanmin(F4),np.nanmax(F4)); print('f4X old/new range',np.nanmin(f4X[ids]),np.nanmax(f4X[ids]),np.nanmin(F4X),np.nanmax(F4X),'delta median/max',np.median(abs(F4X-f4X[ids])),np.max(abs(F4X-f4X[ids])))
...
out.to_csv('/mnt/data/central_A2_f4X_resolved_41.csv',index=False); base.to_csv('/mnt/data/central_A2_f4X_action_jets.csv',index=False)
pd.DataFrame({'u':u[ids],'x':rr,'f4':F4,'f4X_old':f4X[ids],'f4X_new':F4X,'f3_new':F3,'f2F_new':F2F,'f2_new':F2,'f2X_new':F2X,'E00':E00n,'E11':E11n,'JA':JA,'A2_res':res}).to_csv('/mnt/data/central_A2_f4X_background_audit.csv',index=False)
# reducer gates
rows=[]
for L in [6,12,20,42,110,420,1000]:
 ...
 print('L',rows[-1])
pd.DataFrame(rows,columns=['L','Kmin','negK','c2min','negc2']).to_csv('/mnt/data/central_A2_f4X_operator_audit.csv',index=False)
