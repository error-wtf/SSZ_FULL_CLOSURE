...
out.to_csv('/mnt/data/central_A2_resolved_41.csv',index=False)
...
for L in [6,12,20,42,110,420,1000]:
 ...
 print('L',rows[-1])
pd.DataFrame(rows,columns=['L','Kmin','negK','c2min','negc2','h0']).to_csv('/mnt/data/central_A2_operator_audit.csv',index=False)
# source profiles audit
pd.DataFrame({'u':u[central_idx],'x':r_c,'v6':v6new[central_idx],'f3':f3_c,'f2F':f2F_c,'f2':f2_c,'f2X':f2X_c,'E00':E00n,'E11':E11n,'JA':JA,'A2res':v6p-rhs}).to_csv('/mnt/data/central_A2_background_audit.csv',index=False)
