...
from ssz_p5.production.inner_targets import JET_ORDERS, endpoint_derivative_orders
...
def main():
    ...
    b=np.concatenate([desired[k]-ref[k].to_numpy(float) for k in CH5])
    # Bind only the endpoint jets required by the frozen target contract.
    # Never add derivative conditions here by hand: JET_ORDERS is the single
    # source of truth (currently e3 has order 0, i.e. endpoint value only).
    Ae3=maps['e3']
    et=lo.e3_target.to_numpy(float)-ref.e3.to_numpy(float)
    endpoint_rows=[]; endpoint_rhs=[]
    Dpow=sparse.eye(n, format='csr')
    target_deriv=et.copy()
    for derivative_order in endpoint_derivative_orders('e3'):
        if derivative_order > 0:
            Dpow = Dpow @ D
            target_deriv = np.asarray(D @ target_deriv).ravel()
        response = Dpow @ Ae3
        for i in (0,n-1):
            endpoint_rows.append(response.getrow(i))
            endpoint_rhs.append(float(target_deriv[i]))
    ...
    errs={}
    for k in CH5:
        got=ref[k].to_numpy(float)+pred[k]; tgt=desired[k]
    ...
    epred=ref.e3.to_numpy(float)+pred['e3']; etarget=lo.e3_target.to_numpy(float)
    ej=[]
    arrp=epred.copy(); arrt=etarget.copy()
    for derivative_order in endpoint_derivative_orders('e3'):
        if derivative_order > 0:
            arrp=np.asarray(D@arrp).ravel(); arrt=np.asarray(D@arrt).ravel()
    ...
    errs['e3_endpoint_jets']={'max_scaled':float(max(ej)),'median_scaled':float(np.median(ej)),
                              'required_orders':list(endpoint_derivative_orders('e3'))}
    ...
    final=ref.copy()
    for k in SLOT_NAMES:
        final[k]=ref[k].to_numpy(float)+(em[k].to_numpy(float)-eb[k].to_numpy(float))
    ...
    final['construction']='diagnostic historical baseline + direct holonomic 14-profile action deformation; e3 interior free; endpoint orders from JET_ORDERS'
    final['certification_eligible']=False
    ...
    completed.to_csv(OUT/'INNER_COMMON_ACTION_JETS_FREE_E3.csv',index=False)
    final.to_csv(OUT/'INNER_DIRECT_41_FREE_E3.csv',index=False)
    qdf=bg[['u','x']].copy()
    ...
    qdf.to_csv(OUT/'INNER_FREE_E3_CONTROLS.csv',index=False)
    report={'status':'PASS' if max(v['max_scaled'] for v in direct_err.values())<1e-5 else 'NUMERICAL_TARGET_REPLAY_OPEN',
      'construction':'same-action 14-profile background-preserving diagnostic; five historical interior channels targeted; e3 endpoints constrained strictly from JET_ORDERS',
      'endpoint_contract':{k:list(endpoint_derivative_orders(k)) for k in JET_ORDERS},
